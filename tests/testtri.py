...
class Renderer:
  ...
  def psort(verticies, rverts, tris, camera, light, width, height):
    ...
    for i in range(len(tris)):
      ...
      if dot3d(n, camRay) < 0 and np.min(trix) > 0 and np.max(trix) < width and np.min(triy) > 0 and np.max(triy) < height: 
        ntris[i] = -camDist
  ...
  def render(self, scene, screen, running, font):
    frame = self.surface.copy()
    frame[:,:,0], frame[:,:,1], frame[:,:,2] = scene.background
    ...
    for object in scene.objects:
      ...
      for i in np.argsort(zsort):
        ...
        triangle = np.asarray([rverts[object[0].faces[i][0]][:2], rverts[object[0].faces[i][1]][:2], rverts[object[0].faces[i][2]][:2]]).astype(np.uint16)
        print(triangle)
        #color = [0,255,80]
        #pg.draw.polygon(frame, color, [rverts[object[0].faces[i][0]][:2], rverts[object[0].faces[i][1]][:2], rverts[object[0].faces[i][2]][:2]])
        self.drawtriangle(frame, triangle, color)
        screen.blit(pg.surfarray.make_surface(self.surface), (0,0))
        screen.blit(pg.surfarray.make_surface(frame), (0, 0))
        coord = font.render(str(triangle).replace("\n", ", "), False, (0, 0, 0))
...
def main():
  ...
  while running:
    ...
    renderer.render(scene, screen, running, font)
    """
