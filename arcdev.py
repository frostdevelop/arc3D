...
class Renderer:
  ...
  @staticmethod
  @njit()
  def project(verticies, projection, centerx, centery):
    #multiply only neccessary for projection
    vertlen = len(verticies)
    ...
    for i in range(vertlen):
      if verticies[i][2] < 0:
        ...
        nverts[i][1] = nverts[i][0]
        continue
      nverts[i][1] = int(((verticies[i][1]*projection[1, 1])/verticies[i][2] + 1) * centery)
      nverts[i][0] = int(((verticies[i][0]*projection[0, 0])/verticies[i][2] + 1) * centerx)
