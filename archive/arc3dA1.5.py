...
class Scene:
  def __init__(self, objects, background, texture):
...
class Renderer:
  def __init__(self, width, height, camera, light):
    ...
    self.uv = np.asarray([[0.5,0], [0,1], [1,1]]).astype(np.float32)
    """
    self.projection[0,0] = 1/(np.tan(self.camera.vfov/2)*(self.width/self.height))
      self.projection[1,1] = 1/np.tan(self.camera.vfov/2)
  ...
  @staticmethod
  @njit()
  def rendertris(verticies, rverts, tris, camera, light, width, height, frame, colscale):
    ntris = np.zeros(len(tris))
    shade = ntris.copy()
    for i in range(len(tris)):
      tri = tris[i]
    ...
    for i in np.argsort(ntris):
      ...
      tri = np.asarray([list(rverts[tris[i][0]][:2]), list(rverts[tris[i][1]][:2]), list(rverts[tris[i][2]][:2])]).astype(np.int16)
  ...
  def render(self, scene):
    ...
    for object in scene.objects:
      ...
      """
      self.rendertris(overts, rverts, object[0].faces, self.camera.position, self.light, self.width, self.height, frame, colscale)
    self.output = frame
...
def main():
  ...
  scene = Scene([[teapot,[0,0,0]]],[50,127,200], "pfpatlas.png")
