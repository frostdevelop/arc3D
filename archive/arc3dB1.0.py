  ...
  def __init__(self, verts, faces, mat, **kwargs):
    ...
    if mat == 1 or mat == 2:
      self.tex = pg.surfarray.array3d(pg.image.load(kwargs.get(
          "tex", ""))).astype(np.uint8)
      self.texsize = np.array([len(self.tex) - 1,
                               len(self.tex[0]) - 1]).astype(np.uint16)
    else:
      self.tex = np.empty((1, 3), dtype=np.uint8)
      self.texsize = np.empty(2, dtype=np.uint16)
    if mat == 2:
      self.texmap = np.array(kwargs.get("texmap",
                                        np.empty(1))).astype(np.uint32)
      self.texcoord = np.array(kwargs.get("texcoord",
                                          np.empty(1))).astype(np.float32)
    else:
...
class Renderer:
            ...
            for x in range(xc1, xc2):
              ...
              if z < zbuffer[x, y]:
                ...
                if min(uvo) >= 0 and max(uvo) <= 1:
                  zbuffer[x, y] = z
                  if mat == 1 or mat == 2:
                    frame[x, y] = shade[i] * tex[int(uvo[0] * texsize[0]),
                                                 int(uvo[1] * texsize[1])] * (
                                                     max(0, 100 - z) / 100)
  ...
  def project3(verticies, projection, centerx, centery):
    ...
    for i in range(len(verticies)):
      ...
      if nvert[3] >= 0:
        nvert = np.append(np.divide(nvert[:3], nvert[3]),
                          nvert[3] + 1e-16).astype(np.float32)
      nvert[1] = -nvert[1]
      nvert[0] += 1
      nvert[1] += 1
      nvert[0] *= centerx
  ...
  def render(self, scene):
    ...
    for object in scene.objects:
      ...
      nplane = np.asarray((self.camera.position+np.asarray(self.rotatem(self.nearplane[0], self.camera.yang,self.camera.xang),np.float32),np.asarray(self.rotatem(self.nearplane[1],self.camera.yang,self.camera.xang),np.float32)),np.float32)
      ...
      if rmat != 2:
        self.rendertris(overts, rverts, rfaces, self.camera.position,scene.light, self.width, self.height, self.surface,self.zbuffer, rmat, object[0].tex,object[0].texsize, self.uv)
      else:
        self.rendertris(overts, rverts, rfaces, self.camera.position, scene.light, self.width, self.height, self.surface, self.zbuffer, rmat, object[0].tex, object[0].texsize,np.empty((3, 2), dtype=np.float32), rmap, rcoord)
  ...
  def rotatem(vert, yang, xang):
    tx = [
      vert[0] * np.cos(yang) + vert[2] * np.sin(yang), 1,
      vert[2] * np.cos(yang) - vert[0] * np.sin(yang)
    ]
    ty = [
      1, vert[1] * np.cos(xang) - vert[2] * np.sin(xang),
      vert[1] * np.sin(xang) + vert[2] * np.cos(xang)
    ]
    return np.asarray([tx[0]*ty[0],tx[1]*ty[1],tx[2]*ty[2]],np.float32)
  ...
  def cliptri(plane, mat, uv, faces, verts, coord, texmap):
    ...
    for i in range(len(faces)):
      ...
      match voutn:
        case 0:
          nfaces = np.append(nfaces, np.asarray([[face[0],face[1],face[2]]]), axis=0)
          if mat == 2 or mat == 1:
            nmap = np.append(nmap, np.asarray([[texmap[i][0],texmap[i][1],texmap[i][2]]]), axis=0)
          continue
        case 1:
          ...
          nfaces = np.append(nfaces, np.asarray([[nt[0],nt[1],nt[2]]]), axis=0)
          if mat == 2 or mat == 1:
          ...
          nfaces = np.append(nfaces, np.asarray([[nvt[0],nvt[1],nvt[2]]]), axis=0)
          if mat == 2 or mat == 1:
            ntex = t * (coord[texmap[i, vin[1]]] - coord[texmap[i, vout[0]]]) + coord[texmap[i, vin[1]]]
            coord = np.append(coord, np.asarray([[ntex[0],ntex[1]]]),axis=0)
            nvm[vin[1]] = len(coord) - 1
            nmap = np.append(nmap, np.asarray([[nvm[0],nvm[1],nvm[2]]]), axis=0)
