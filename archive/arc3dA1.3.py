...
class Renderer:
  def __init__(self, width, height, camera, light, surface):
    ...
    self.light = light
    self.projection = np.array([[(self.height/self.width)*(1/np.tan(self.camera.vfov/2)),0,0,0],
      [0,1/np.tan(self.camera.vfov/2),0,0],
      [0,0,-self.camera.zfar/(self.camera.zfar-self.camera.znear),1],
      [0,0,-(self.camera.zfar*self.camera.znear)/(1000/0.1),0]])
    """
    self.projection = np.array([[1/(np.tan(self.camera.vfov/2)*(self.width/self.height)),0,0,0],
      [0,1/np.tan(self.camera.vfov/2),0,0],
      [0,0,-(self.camera.zfar+self.camera.znear)/(self.camera.zfar-self.camera.znear),-1],
      [0,0,-(self.camera.zfar*self.camera.znear*2)/(1000/0.1),0]])
    """
    """
    self.projection = np.array([[1/(np.tan(self.camera.vfov/2)*(self.width/self.height)),0,0,0],
      [0,1/np.tan(self.camera.vfov/2),0,0],
...
def main():
  ...
  #camera = Camera(np.pi/2,[0,0,0],0,0, width, height)
