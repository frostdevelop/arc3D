...
class Renderer:
  ...
  @staticmethod
  @njit()
  def rendertris(verticies,rverts,tris,camera,light,width,height,frame,zbuffer,mat,tex,texsize,iuv=np.empty((3, 2), dtype=np.float32),texmap=np.empty((1, 3), dtype=np.uint16),texcoord=np.empty((1, 2), dtype=np.float32)):
    if mat == 0:
    ...
    for i in range(len(tris)):
      ...
      if dot3d(n, camRay) < 0 and zmin > 0 and (
        (trix[0] >= -width and trix[0] <= 2 * width and triy[0] >= -height and triy[0] <= 2 * height) or
        (trix[1] >= -width and trix[1] <= 2 * width and triy[1] >= -height and triy[1] <= 2 * height) or
        (trix[2] >= -width and trix[2] <= 2 * width and triy[2] >= -height and triy[2] <= 2 * height)):
        ...
        zstart, zmiddle, zend = 1 / (zstart + 1e-32), 1 / (zmiddle + 1e-32), 1 / (zend + 1e-32)
  ...
  def project3(verticies, projection, centerx, centery):
    ...
    for i in range(len(verticies)):
      ...
      if nvert[3] >= 0:
        ...
        nvert = np.empty(4, dtype=np.float32)
      nvert[1] = -nvert[1]
      nvert[0] += 1
      nvert[1] += 1
      nvert[0] *= centerx
  ...
  def render(self, scene):
    ...
    for object in scene.objects:
      ...
      """
      if object[0].mat != 2:
        self.rendertris(overts, rverts, object[0].faces, self.camera.position, scene.light, self.width, self.height, self.surface, self.zbuffer, object[0].mat, object[0].tex, object[0].texsize, self.uv)
      else:
        self.rendertris(overts, rverts, object[0].faces, self.camera.position, scene.light, self.width, self.height, self.surface, self.zbuffer, object[0].mat, object[0].tex, object[0].texsize, np.empty((3, 2), dtype=np.float32), object[0].texmap, object[0].texcoord)
...
def main():
  ...
  scube = Object(np.asarray([[-2, -2, -2], [2, 2, -2], [-2, 2, -2],
                             [2, -2, -2], [-2, -2, 2], [2, 2, 2], [-2, 2, 2],
                             [2, -2, 2]], dtype=np.float32),
                 np.array([[0, 2, 3], [2, 1, 3], [7, 5, 4], [5, 6,
                                                             4], [4, 6, 0],
                           [6, 2, 0], [3, 1, 7], [1, 5, 7], [7, 4, 0],
                           [0, 3, 7], [2, 6, 1], [6, 5, 1]], dtype=np.uint16),
                 2,
                 tex="models/sheepy.png",
                 texcoord=[[0, 0], [0, 1], [1, 0], [1, 1]],
                 texmap=[[0, 1, 2], [1, 3, 2], [0, 1, 2], [1, 3, 2], [0, 1, 2],
                         [1, 3, 2], [0, 1, 2], [1, 3, 2], [2, 0, 1], [1, 3, 2],
                         [0, 1, 2], [1, 3, 2]])
  iverts, itris = readobj("models/teapot.obj")
  teapot = Object(iverts,
                  itris,
                  1,
                  tex="models/stole.png")
  iverts, itris = readobj("models/mountains.obj")
  ...
  scene = Scene(
      [[croc, np.asarray([0, 10, 0],dtype=np.float32)],
       [mountains, np.asarray([0, -10, 0], dtype=np.float32)]], light,
      [50, 127, 200])
  ...
  camera = Camera(np.pi / 2, np.asarray([0.0, 1.5, -5.0], dtype=np.float32), 0, 0, 1000, 0.1, width, height)
